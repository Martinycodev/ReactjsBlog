...
var controller = {
    ...
    save: (req, res) => {
        ...
        if (validate_title && validate_content) {
            ...
            article.save((err, article) => {
                ...
                if (err || !article) {
                    return res.status(404).send({ status: 'error', message: 'los datos no se han guardado' });
                } else {
                    //respuesta
    ...
    getArticles: (req, res) => {
        ...
        query.sort('-_id').exec((err, articles) => {
            ...
            if (err) {
                return res.status(500).send({ status: 'error', message: 'los datos no son válidos' });
            }

            if (!articles) {
                return res.status(404).send({ status: 'error', message: 'no hay articulos que mostrar' });
            }
